...
namespace MineSnake
{
    public class MSSnakeAI
    {
        ...
        public List<MSDirection> getDirections(MSGrid _grid)
        {
            ...
            for (i = 0; i < h; i++)
            {
                for (j = 0; j < w; j++)
                {
                    ...
                    if (!grid[i][j].isOf(MSTileProperty.OBSTACLE) && !grid[i][j].isOf(MSTileProperty.SNAKE))
                        matrix[i][j] = 0;
                    else
                        matrix[i][j] = 1;

                    if (grid[i][j].isOf(MSTileProperty.START))
                    {
                        xstart = i;
                        ystart = j;
                    }
